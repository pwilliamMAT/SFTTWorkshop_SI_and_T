...
void b_sort(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  nBlocks = x->size[1];
  idx->size[1] = nBlocks;
  emxEnsureCapacity_int32_T(&st, idx, i1, &ki_emlrtRTEI);
  idx_data = idx->data;
  for (k = 0; k < nBlocks; k++) {
    idx_data[k] = 0;
  }
  if (x->size[1] != 0) {
    ...
    emxEnsureCapacity_int32_T(&b_st, iwork, i1, &uh_emlrtRTEI);
    iwork_data = iwork->data;
    for (k = 0; k < nBlocks; k++) {
      iwork_data[k] = 0;
    }
    emxInit_real_T(&b_st, &xwork, 1, &yh_emlrtRTEI);
    i1 = xwork->size[0];
...
void sort(const emlrtStack *sp, emxArray_uint32_T *x)
{
  ...
  emxInit_uint32_T(sp, &vwork, 1, &wh_emlrtRTEI);
  b_i = vwork->size[0];
  vwork->size[0] = i;
  ...
  emxInit_int32_T(sp, &iidx, 1, &xh_emlrtRTEI);
  ...
  for (j = 0; j < vstride; j++) {
    ...
    emxEnsureCapacity_int32_T(&st, iidx, dim, &th_emlrtRTEI);
    iidx_data = iidx->data;
    for (k = 0; k < b_i; k++) {
      iidx_data[k] = 0;
    }
