                                   ...
                                   int32_T numMeas, int32_T numTracks)
{
  ...
  emlrtStack c_st;
  emlrtStack st;
  emxArray_real_T *b_validationMatrix;
  real_T x_data[51];
  real_T n;
  real_T n1;
  real_T n2;
  real_T *b_validationMatrix_data;
  int32_T k;
  int32_T maxval;
  int32_T nPotentials;
  ...
  n1 = 1.0;
  nPotentials = numMeas;
  if (numMeas > numTracks) {
    nPotentials = numTracks;
  }
  maxval = numMeas;
  if (numMeas < numTracks) {
    maxval = numTracks;
  ...
  b_validationMatrix->size[1] = validationMatrix->size[1];
  emxEnsureCapacity_real_T(&st, b_validationMatrix, nPotentials, &dg_emlrtRTEI);
  b_validationMatrix_data = b_validationMatrix->data;
  nPotentials = validationMatrix->size[0] * validationMatrix->size[1];
  for (k = 0; k < nPotentials; k++) {
    b_validationMatrix_data[k] = validationMatrix_data[k];
  }
  b_st.site = &xcb_emlrtRSI;
  ...
  n2 = muDoubleScalarRound(muDoubleScalarMin(n1, n2));
  if (n2 < 2.147483648E+9) {
    if (n2 >= -2.147483648E+9) {
      nPotentials = (int32_T)n2;
    } else {
      nPotentials = MIN_int32_T;
    }
  } else if (n2 >= 2.147483648E+9) {
    nPotentials = MAX_int32_T;
  } else {
    nPotentials = 0;
  }
...
/* End of code generation (numPotentialFeasibleEvents.c) */
