...
for i = 1:numel(tuningData)
    ...
    % Find the first index strictly beyond scenarioDuration
    idxPast = find(time > scenarioDuration, 1, 'first');

    % If we never exceed scenarioDuration, use all samples; otherwise 1:idxPast
    if isempty(idxPast)
        lastIdx = numel(time);
    else
        lastIdx = idxPast;
    ...
    % Build logical mask: all samples up to and including the first beyond
    validIdx = false(size(time));
    validIdx(1:lastIdx) = true;
